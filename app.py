...
def get_admin_history():
    sessions = sessions_collection.find({"status": "completed"}).sort(
        "entry_time", -1
    )
    data = []
    for s in sessions:
...
def gate_state():
    now = datetime.now(IST)
    state = gate_state_collection.find_one({"_id": "main_gate"}) or {}
    if not state:
        state = {
            "mode": "entry",
        ...
        gate_state_collection.insert_one({"_id": "main_gate", **state})
    return jsonify(
        {
            "mode": state.get("mode", "entry"),
...
def gate_update():
    ...
    if mode not in {"entry", "exit"}:
        return jsonify({"message": "mode must be entry or exit"}), 400
    message = payload.get("message") or (
        "Vehicle detected. Scan QR / Open entry form"
        if mode == "entry"
        else "Payment received. Opening gate"
    )
    vehicle_no = normalize_vehicle_no(payload.get("vehicle_no", ""))
    gate_state_collection.update_one(
        {"_id": "main_gate"},
